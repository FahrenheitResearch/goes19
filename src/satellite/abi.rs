...
use super::netcdf::{
    ScaledVariable, dataset_attr_f64, dataset_attr_string, open_goes_netcdf_lossy, read_scaled_f32,
    read_scaled_f32_window,
};
...
pub fn read_goes_abi_scene(path: impl AsRef<Path>) -> Result<GoesAbiScene, Box<dyn Error>> {
    ...
    let projection_var = file.dataset("goes_imager_projection")?;
    let projection = GoesImagerProjection {
        ...
        sweep_angle_axis: dataset_attr_string(&projection_var, "sweep_angle_axis")
            .as_deref()
...
fn required_attr_f64(variable: &rustyhdf5::Dataset<'_>, name: &str) -> Result<f64, Box<dyn Error>> {
    dataset_attr_f64(variable, name)
